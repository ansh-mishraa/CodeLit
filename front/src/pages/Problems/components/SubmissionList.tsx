...
const SubmissionsList = ({ submissions, isLoading }: { submissions: any; isLoading: boolean }) => {
  ...
  const calculateAverageMemory = (memoryData: any) => {
    const memoryArray = safeParse(memoryData).map((m:any) =>
      parseFloat(m.split(" ")[0])
    );
    if (memoryArray.length === 0) return 0;
    return (
      memoryArray.reduce((acc:any, curr:any) => acc + curr, 0) / memoryArray.length
    );
  };

  // Helper function to calculate average runtime
  const calculateAverageTime = (timeData:any) => {
    const timeArray = safeParse(timeData).map((t:any) =>
      parseFloat(t.split(" ")[0])
    );
    if (timeArray.length === 0) return 0;
    return timeArray.reduce((acc:any, curr:any) => acc + curr, 0) / timeArray.length;
  };
